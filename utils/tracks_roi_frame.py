...
def roi_midpoint(frame_pos):
    roi_path = "/data/aicity/aic23/track4/PersonGONE/data/ROIs/testA_1/"
...
# compute time of track detection
    ...
    for t in tracks:
        ...
        for frame in tracks_time[t]['frames_nums']:
            detections_of_class = [det for det in tracks[t] if det['class'] == tracks_classes[t]['class']]
            for det in detections_of_class:
                distances.append(np.linalg.norm(np.array(det['pos']) - np.array(roi_midpoint(frame))))
            closest_detection_index = np.argmin(distances)
            closest_frame = tracks_time[t]['frames_nums'][closest_detection_index]
